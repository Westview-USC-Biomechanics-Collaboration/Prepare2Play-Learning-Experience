...
def process(self, *value):
    try:
        # Get requested slider value
        requested_loc = self.slider.get()
        
        # Clamp to safe bounds
        self.state.loc = self.state.get_safe_slider_value(requested_loc)
        
        # Update slider if it was clamped
        if self.state.loc != requested_loc:
            self.slider.set(self.state.loc)
            print(f"[DEBUG] Clamped slider from {requested_loc} to {self.state.loc}")
        ...
        if self.state.video_loaded:
            ...
            self.canvasManager.photo_image1 = self.frameConverter.cvToPillow(
                camera=self.Video.cam, 
                width=round(self.Video.frame_width * self.canvasManager.zoom_factor1),
                height=round(self.Video.frame_height * self.canvasManager.zoom_factor1), 
                frame_number=self.state.loc
            )
        ...
        try:
            if self.state.vector_overlay_enabled:
                # For vector overlay, use the same frame number
                # (trimmed video already accounts for offset)
                ...
                if self.state.com_enabled:
                    ...
                    if rawFrame is not None:
                        ...
                        self.canvasManager.photo_image3 = self.frameConverter.cvToPillowFromFrame(
                            COMFrame,
            ...
            print("[ERROR] Index out of range, check pose_landmarks.csv file to verify rows")
            traceback.print_exc()
        ...
        if self.state.force_loaded:
            # Draw graph canvas
            try:
                plate_number = "1" if self.backgroundManager.plate.get() == "Force Plate 1" else "2"
                
                # Calculate the force data index
                force_idx = int(self.state.loc * self.state.step_size + self.state.zoom_pos)
                
                # Clamp force index to valid range
                force_idx = max(0, min(force_idx, len(self.Force.data) - 1))
                
                x_position = float(self.Force.data.iloc[force_idx, 0])
                y_value = float(self.Force.data.loc[force_idx, f"{self.backgroundManager.option.get()}{plate_number}"])
                
                # Update baseline and current line
                baseline_idx = self.state.loc * self.state.step_size
                baseline_idx = max(0, min(baseline_idx, len(self.Force.data) - 1))
                
                self.zoom_baseline.set_xdata([self.Force.data.iloc[baseline_idx, 0]])
                ...
                print(f"[WARNING] Force data out of range at frame {self.state.loc}: {e}")
