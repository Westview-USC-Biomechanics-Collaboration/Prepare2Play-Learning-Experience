"""
Updated ledSyncing.py - Integrates new LED detection system

Replace the contents of GUI/callbacks/ledSyncing.py with this code.
"""
...
from GUI.callbacks.led_detection_system import process_view

plt.ioff()


def align_data(df_f, df_v):
    """
    Align force and video data using LED signals.
    
    Args:
        df_f: Force dataframe with FP_LED_Signal
        df_v: Video dataframe with Video_LED_Signal
        
    Returns:
        df_aligned: Aligned dataframe
        lag: Frame offset
        max_corr: Maximum correlation value
        perfect_corr: Perfect correlation value
        relative_score: Correlation quality metric
    """
    force_step = 10
...
def new_led(self, view, parent_path, video_file, force_file):
    """
    NEW IMPLEMENTATION: Uses the improved LED detection system.
    
    This function:
    1. Detects LED in video using robust template matching
    2. Extracts LED signal from video
    3. Processes force file to get force LED signal
    4. Aligns video and force data
    5. Saves results
    
    Args:
        self: GUI app instance
        view: Camera view ("Long View", "Top View", or "Short View")
        parent_path: Path to data folder
        video_file: Name of video file
        force_file: Name of force file
        
    Returns:
        lag: Frame offset for alignment
        df_aligned: Aligned dataframe with video and force data
    """
    print(f"[INFO] Processing {view} with new LED detection system...")
    ...
    output_led = os.path.join(parent_path, 'Output_LED')
    os.makedirs(output_led, exist_ok=True)
    
    # ========================================================================
    # STEP 1: Extract LED signal from video using new detection system
    # ========================================================================
    print(f"[INFO] Detecting LED and extracting signal from video...")
    df_video = process_view(path_video, view, output_led)
    
    # ========================================================================
    # STEP 2: Process force file
    # ========================================================================
    print(f"[INFO] Processing force file...")
    ...
    for key, value in force_dict.items():
        if key in df_force.columns:
            df_force.rename(columns={key: value}, inplace=True)
    
    # Create LED signal from force plate 3
    df_force['FP_LED_Signal'] = np.sign(df_force['FP3_Fz'])
    
    # Save processed force file
    force_analysis_filename = force_file.replace('.txt', '_Analysis_Force.csv')
    force_output_path = os.path.join(parent_path, force_analysis_filename)
    df_force.to_csv(force_output_path, index=False)
    print(f"[INFO] Saved processed force data to: {force_output_path}")
    
    # ========================================================================
    # STEP 3: Align video and force data
    # ========================================================================
    print(f"[INFO] Aligning video and force data...")
    df_aligned, lag, max_corr, perfect_corr, relative_score = align_data(df_force, df_video)
    
    # ========================================================================
    # STEP 4: Save alignment results
    # ========================================================================
    ...
    print(f"[RESULTS] Saved alignment results to: {df_result_path}")
    print(f"[RESULTS] Lag={lag}, Max Correlation={max_corr:.2f}, "
          f"Perfect={perfect_corr}, Relative Score={relative_score:.4f}")
    
    # ========================================================================
    # STEP 5: Create alignment visualization
    # ========================================================================
    print(f"[INFO] Creating alignment visualization...")
    _create_alignment_plot(df_aligned, df_force, df_video, lag, parent_path)
    
    # Save lag to file for other parts of code
...
def _create_alignment_plot(df_aligned, df_force, df_video, lag, parent_path):
    """
    Create visualization showing video and force signals aligned.
    
    Args:
        df_aligned: Aligned dataframe
        df_force: Force dataframe
        df_video: Video dataframe
        lag: Frame offset
        parent_path: Where to save plot
    """
    try:
        # Downsample force signal to match video frame rate
        force_step = 10
        df_force_subset = df_force.iloc[::force_step].reset_index(drop=True)
        
        signal_video = df_video['Video_LED_Signal'].values
        signal_force = df_force_subset['FP_LED_Signal'].values
        
        # Apply lag to align signals
        if lag >= 0:
            force_pad = np.concatenate([np.zeros(lag), signal_force])
        else:
            force_pad = signal_force[abs(lag):]
        
        # Trim to same length
        n = min(len(signal_video), len(force_pad))
        sig_video = signal_video[:n]
        sig_force = force_pad[:n]
        ...
        x = np.arange(n)
        ax.step(x, sig_video, where='mid', alpha=0.6, label="Video Signal", linewidth=1.5)
        ax.step(x, sig_force, where='mid', alpha=0.6, label="Force Signal", linewidth=1.5)
        
        ax.set_title(f"LED Signal Alignment (Lag = {lag} frames)")
        ...
        plot_path = os.path.join(parent_path, "led_alignment_plot.png")
        fig.savefig(plot_path, dpi=150, bbox_inches="tight")
        print(f"[INFO] Saved alignment plot to: {plot_path}")
        
    except Exception as e:
        print(f"[WARNING] Could not create alignment plot: {e}")


# Keep the old run_led_syncing function as fallback (optional)
def run_led_syncing(self, parent_path, video_file, force_file):
    """
    OLD IMPLEMENTATION: Kept as fallback.
    
    If you have issues with the new system, you can temporarily switch back
    to this function by calling it instead of new_led().
    """
    # ... (keep existing run_led_syncing code as backup)
    pass
