...
import pandas as pd
from Util.windows_file_dialog import save_file_dialog
...
def saveCallback(self):
    ...
    def _label(com_in):
        if com_in < 0:  # label start
            ...
            self.EndLabel.config(text=f"end frame: {self.save_end}")

    def _scrollBar(value):
        # Get requested value
        requested_loc = self.save_scroll_bar.get()
        
        # Clamp to valid bounds
        min_bound, max_bound = self.state.get_slider_bounds()
        self.save_loc = max(min_bound, min(requested_loc, max_bound))
        
        # Update scroll bar if clamped
        if self.save_loc != requested_loc:
            self.save_scroll_bar.set(self.save_loc)
        
        print(f"Save window scroll bar moved to {self.save_loc}")
        
        self.Video.vector_cam.set(cv2.CAP_PROP_POS_FRAMES, self.save_loc)
        if self.Video.frame_width > self.Video.frame_height:
            self.save_photoImage = self.frameConverter.cvToPillow(
    ...
    def _export():
        ...
        if self.save_start < min_bound or self.save_start > max_bound:
            print(f"[ERROR] Start frame {self.save_start} is out of bounds [{min_bound}, {max_bound}]")
            self._pop_up(f"Start frame out of bounds!\nValid range: {min_bound} to {max_bound}")
            return
        
        if self.save_end < min_bound or self.save_end > max_bound:
            print(f"[ERROR] End frame {self.save_end} is out of bounds [{min_bound}, {max_bound}]")
            self._pop_up(f"End frame out of bounds!\nValid range: {min_bound} to {max_bound}")
            return
        
        if self.save_start >= self.save_end:
            print(f"[ERROR] Start frame ({self.save_start}) must be less than end frame ({self.save_end})")
            self._pop_up("Start frame must be before end frame!")
            return
        ...
        # Determine force column names based on view
        ...
        force_cols = [label1_1, label1_2, label2_1, label2_2]
        ...
        count = start_abs
        count_vid = self.save_start - cushion_frames
        ...
        def render_matplotlib_to_cv2(cur):
            cur = np.clip(cur, 9, dfw.shape[0] - 1)
            ...
            if self.Video.frame_width > self.Video.frame_height:
                gap_width = (1920 - total_width) // 2
                gap = np.full((image1.shape[0], gap_width, 3), 255, dtype=np.uint8)
                ...
                return cv2.vconcat([gap, image1, image2, gap])
        ...
        if self.Video.frame_width > self.Video.frame_height:
            out = cv2.VideoWriter(
                ...
                out_fps,
                (self.Video.frame_width, self.Video.frame_height + 480)
            )
        else:
            out = cv2.VideoWriter(
                file_path, 
                cv2.VideoWriter_fourcc(*'mp4v'), 
                out_fps,
                (self.Video.frame_width + 640, self.Video.frame_height)
            )
        ...
        while self.Video.vector_cam.isOpened() and count_vid <= end_f:
            ...
            # Apply COM if checkbox is checked
            if self.COM_intVar.get() == 1:
                try:
                    # Use OUTPUT frame index (0-based from trimmed video)
                    # NOT count_vid which is original video frame
                    output_frame_idx = count_vid - self.save_start
                    frame3 = self.COM_helper.drawFigure(frame3, output_frame_idx)
                except IndexError as e:
                    # Only print first few errors
                    if count_vid - self.save_start < 5:
                        print(f"[WARN] COM data not available for frame {output_frame_idx}: {e}")
                except Exception as e:
                    print(f"[ERROR] Unexpected error drawing COM: {e}")
            ...
            if count_vid < self.save_start:
                combined_frame = cv2.vconcat([frame1, graphs]) if self.Video.frame_width > self.Video.frame_height else cv2.hconcat([frame1, graphs])
            elif count_vid <= self.save_end:
                combined_frame = cv2.vconcat([frame3, graphs]) if self.Video.frame_width > self.Video.frame_height else cv2.hconcat([frame1, graphs])
            else:
                combined_frame = cv2.vconcat([frame1, graphs]) if self.Video.frame_width > self.Video.frame_height else cv2.hconcat([frame1, graphs])
        ...
        self._pop_up(text=f"Successfully saved video at {file_path}")
        print(f"Successfully saved video at {file_path}")
    ...
    # Local variables
    self.save_loc = self.state.slider_min  # Start at minimum valid frame

    # Get valid bounds for save window
    min_bound, max_bound = self.state.get_slider_bounds()
    ...
    # Use proper bounds for save scroll bar
    self.save_scroll_bar = tk.Scale(
        self.save_window, 
        from_=min_bound,  # Use state bounds
        to=max_bound,     # Use state bounds
        orient="horizontal", 
        label=f"select start and end (frames {min_bound}-{max_bound})", 
        command=_scrollBar
    )
